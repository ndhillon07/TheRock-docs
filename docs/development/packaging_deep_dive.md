...
#### The Complete Chain: Python → therock_add_feature() → CMake Variables

**This shows the exact mechanism that converts TOML to command-line flags.**

**STEP 1: Python reads TOML and writes CMake function calls**

```python
# File: build_tools/topology_to_cmake.py (simplified)
...
def generate_feature_declarations(topology: BuildTopology, f: TextIO):
    ...
    for artifact in topology.get_artifacts():
        ...
        # ↑ Example: "BLAS" (from artifact.feature_name)

        feature_group = topology.get_artifact_feature_group(artifact)
        # ↑ Example: "MATH_LIBS" (from artifact.feature_group)
        ...
        # ↑ Example: ["HIP_RUNTIME"] (from artifact.artifact_deps)

        # GENERATE THE CMAKE CODE - this writes to the output file!
...
**STEP 2: Python script outputs generated CMake code to a file**

When you run `cmake -B build`, CMake calls `topology_to_cmake.py` which creates this file:

```cmake
# File: build/cmake/therock_topology_generated.cmake
# (Auto-generated by topology_to_cmake.py - DO NOT EDIT)

# This is what the Python script wrote to the file:
...
therock_add_feature(SOLVER
  GROUP MATH_LIBS
  DESCRIPTION "Enables solver"
  REQUIRES HIP_RUNTIME BLAS
)
```

**STEP 3: CMake executes therock_add_feature() function**

The main CMakeLists.txt includes the generated file:

```cmake
# Main CMakeLists.txt
include(build/cmake/therock_topology_generated.cmake)
# ↑ This executes all the therock_add_feature() calls above
```

**STEP 4: therock_add_feature() creates CMake cache variables**

Here's what the `therock_add_feature()` function actually does (defined in `cmake/therock_features.cmake:22`):
...
function(therock_add_feature feature_name)
  ...
  # Determine default value (ON or OFF)
  set(_default_enabled ON)
  if(ARG_GROUP)
    # If this feature belongs to a group, default is OFF
    # unless the group is enabled
    if(NOT "${THEROCK_ENABLE_${ARG_GROUP}}")
      set(_default_enabled OFF)
    endif()
  endif()

  # Check if disabled on current platform
  ...
  # CREATE THE CACHE VARIABLE - this is the key part!
  set(THEROCK_ENABLE_${feature_name} ${_default_enabled}
      CACHE BOOL "${ARG_DESCRIPTION}")
  # ↑ This creates a variable you can override with -D flag!

  # Export to parent scope
  set(THEROCK_ENABLE_${feature_name} "${_actual}" PARENT_SCOPE)
  set(THEROCK_REQUIRES_${feature_name} ${ARG_REQUIRES} PARENT_SCOPE)

  # Add to global list of all features
  list(APPEND _all_features "${feature_name}")
  set(THEROCK_ALL_FEATURES ${_all_features} PARENT_SCOPE)
endfunction()
```

**What `CACHE BOOL` means:**

```cmake
set(THEROCK_ENABLE_BLAS OFF CACHE BOOL "Enables blas")
#                       ↑              ↑
#                       |              |
#                Default value    Shown in cmake-gui and --help

# CACHE means: This variable is saved in build/CMakeCache.txt
# BOOL means: It's an ON/OFF boolean value
# You can override it with: cmake -DTHEROCK_ENABLE_BLAS=ON
```
...
1. CMake starts configuring
   └─ Executes: execute_process(COMMAND python3 topology_to_cmake.py ...)
        └─ Python reads BUILD_TOPOLOGY.toml
        └─ Python writes therock_add_feature() calls to file

2. CMake includes the generated file
   └─ include(build/cmake/therock_topology_generated.cmake)

3. CMake executes each therock_add_feature() call
   └─ therock_add_feature(BLAS GROUP MATH_LIBS ...)
        └─ Creates: THEROCK_ENABLE_BLAS=OFF (default)
        └─ But you overrode it with -DTHEROCK_ENABLE_BLAS=ON
        └─ So final value: THEROCK_ENABLE_BLAS=ON

4. CMake writes to cache file
   └─ build/CMakeCache.txt contains:
      THEROCK_ENABLE_BLAS:BOOL=ON

5. Rest of CMake build system uses this variable
   └─ if(THEROCK_ENABLE_BLAS)
         add_subdirectory(math-libs/BLAS)
      endif()
```

**Complete example showing the full chain:**
...
BUILD_TOPOLOGY.toml:
  ...
  artifact_deps = ["core-hip"]
        ...
        ↓ (read by topology_to_cmake.py)

Python code (topology_to_cmake.py):
  feature_name = "BLAS"                    # From TOML
  feature_group = "MATH_LIBS"              # From TOML
  requires = ["HIP_RUNTIME"]               # From TOML (mapped from core-hip)

  f.write("therock_add_feature(BLAS\n")   # Write to file
  f.write("  GROUP MATH_LIBS\n")
  f.write("  REQUIRES HIP_RUNTIME\n")
  f.write(")\n")

        ↓ (written to build/cmake/therock_topology_generated.cmake)
...
CMake function (therock_features.cmake):
  function(therock_add_feature feature_name)
    # feature_name = "BLAS"
    # ARG_GROUP = "MATH_LIBS"
    # ARG_REQUIRES = "HIP_RUNTIME"

    set(THEROCK_ENABLE_${feature_name} ${_default_enabled} CACHE BOOL ...)
    #   ↑ Creates variable: THEROCK_ENABLE_BLAS
  endfunction()

        ↓ (creates CMake cache variable)

CMake Cache (build/CMakeCache.txt):
  THEROCK_ENABLE_BLAS:BOOL=OFF
  # ↑ Default is OFF because it's in a group (MATH_LIBS)

        ↓ (you override on command line)

Your command:
  cmake -B build -DTHEROCK_ENABLE_BLAS=ON
  #              ↑ This overrides the cache default

        ↓ (final effective value)

Result:
  THEROCK_ENABLE_BLAS=ON  (your override wins)

  CMake build system sees:
    if(THEROCK_ENABLE_BLAS)
      add_subdirectory(math-libs/BLAS)  # ← This executes!
    endif()
```

**Key Insights:**

1. **Python is a code generator** - It doesn't execute CMake logic, it just writes text to a file
2. **therock_add_feature() is a CMake function** - It runs when CMake includes the generated file
3. **CACHE BOOL creates overridable variables** - This is what makes `-D` flags work
4. **The connection:** TOML field `feature_name = "BLAS"` → Python writes `therock_add_feature(BLAS ...)` → CMake creates `THEROCK_ENABLE_BLAS` → You control with `-DTHEROCK_ENABLE_BLAS=ON`
